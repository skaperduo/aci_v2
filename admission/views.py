...
def grad_admission(request):
    ...
    if request.method == "POST":
        # form = StudentPersonalInformationForm(request.POST or None)
        id_pk = GradStudentPersonalInformation.objects.latest('pk')
        school_year = int(request.POST.get('grad_stud_school_year'))
        classification = int(request.POST.get('grad_stud_classification'))
        program = int(request.POST.get('grad_stud_program'))
        year_level = int(request.POST.get('grad_stud_year_level'))
        shift = int(request.POST.get('grad_stud_shift'))
        last_name = request.POST.get('grad_last_name')
        first_name = request.POST.get('grad_first_name')
        middle_name = request.POST.get('grad_middle_name')
        birthdate = request.POST.get('grad_birthdate')
        birth_place = request.POST.get('grad_birth_place')
        religion = request.POST.get('grad_religion')
        nationality = request.POST.get('grad_nationality')
        gender = request.POST.get('grad_gender')
        civil_status = request.POST.get('grad_civil_status')
        email_address = request.POST.get('grad_email_address')
        social_media_accounts = request.POST.get('grad_social_media_accounts')
        mobile_number = request.POST.get('grad_mobile_number')
        landline_number = request.POST.get('grad_landline_number')
        home_address = request.POST.get('grad_home_address')
        where_hear_us = int(request.POST.get('where_hear_us'))
        why_choose_us = int(request.POST.get('why_choose_us'))
        connectivity = request.POST.get('connectivity')
        gadget = request.POST.get('gadget')
        ...
        return render(request, 'grad_admission.html', {
            'programs': all_programs,
            'shifts': all_shifts,
            'year_level': all_year_level,
            'classification': all_classification,
...
def grad_confirmation(request):
    ...
    if request.method == "POST":
        form = GradStudentPersonalInformationForm(request.POST or None)

        grad_last_name = request.POST.get('grad_last_name')
        grad_first_name = request.POST.get('grad_first_name')
        grad_middle_name = request.POST.get('grad_middle_name')
        ref_id_no = request.POST.get('grad_stud_reference_no')

        if form.is_valid():
            form.save()
            subject = "ACI Online Pre-registration Confirmation"
            message = "Dear " + grad_last_name + ", " + grad_first_name + " " + grad_middle_name + \
                      "\n\nWelcome to ACI\n" \
                      "\nWe are delighted to receive your application.\n\n" \
                      "Your registration is confirmed and complete!\n" \
                      "Here is your reference no. (" + ref_id_no + "), for easy reference.\n\n" \
                                                                   ...
                                                                   "Mobile: 09483626888\n"
            from_email = 'aciunofficial@gmail.com'
            email = form['grad_email_address'].value()
            to_list = [email, settings.EMAIL_HOST_USER]
            send_mail(subject, message, from_email, to_list, fail_silently=True)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            message = "Dear " + shs_last_name + ", " + shs_first_name + " " + shs_middle_name + \
                      ...
                      "Your registration is confirmed and complete!\n" \
                      "Here is your reference no. (" + ref_id_no + "), for easy reference.\n\n" \
                                                                   "below are ACI's Official Contact Numbers:\n\n" \
                                                                   "Registrar 342-8004\n" \
                                                                   "Accounting Office 342-5843\n" \
                                                                   "QA 225-2330\n" \
                                                                   "Mobile: 09483626888\n"
...
def jhs_admission(request):
    ...
    if request.method == "POST":
        # form = StudentPersonalInformationForm(request.POST or None)
        id_pk = JuniorHighSchool_StudentPersonalInformation.objects.latest('pk')
...
def elementary_admission(request):
    ...
    if request.method == "POST":
        # form = StudentPersonalInformationForm(request.POST or None)
        id_pk = Elementary_StudentPersonalInformation.objects.latest('pk')
